...
### Full Crucible as the council labeler + a heuristic re-entry watcher — 2026-06-18

- **Chose:** (a) wire the **full belief-revision Crucible** (real multi-Fusant debate → crucible judge) in as an opt-in Agora labeler (`to_agora.py --crucible`); polarity comes from the judge `score`, and **dissent is real** — derived from `unresolved_tension` + `consensus_quality` (herded/resolved/mixed), not faked from the configured disagreeableness dial. (b) Close the loop with a **watcher** (`agora/watcher.py`, the "Hermes on watch"): it measures how hard human verdicts **overturn** the machines (a *lift* = humans rescue a machine-weeded idea; a *drop* = humans kill a machine afrit) and, past a threshold, writes a **respawn ticket** (`tickets.jsonl`) carrying the overturned ideas + a next-seed. A consumer (`spikes/backyard/respawn.py`) re-enters the loop seeded by the human-revealed vein.
- **Over:** (a) keeping only the *light* 3-vote council — rejected per the user's "go full Crucible," though light stays the fast default; (b) putting the respawn TRIGGER in an LLM — rejected: the trigger is pure auditable math so the bus stays dependency-free, and a local Hermes/Pi model may only *enrich* the ticket prose later; (c) a fully-automatic respawn — rejected for now (ticket is written, a human/consumer pulls it), keeping Lα in the loop.
- **Because:** the user wanted to *see* the artifacts and the re-entry, not be told about them. The full Crucible immediately earned its keep: on the seeded hospital-AI idea the single judge said **weed** (score 3) while the full council debated it to a **sprout** (score 7, dissent 0.5) and a human said **novel** — a live three-way divergence, which is exactly the captured signal. The watcher turns that divergence into a falsifiable re-entry decision (RESPAWN on ≥1 lift or divergence ≥ rate).
- **Revisit if:** (a) full-crucible cost/latency is prohibitive at scale (it is slow — multi-call per idea; keep it to small `--limit`, light council for bulk); (b) `spike` and `judge` labelers stay identical (they do: 9/9 = 100% in the first live matrix — drop the redundant `spike` labeler); (c) the watcher's `lifts>=1` trigger proves too trigger-happy at tiny N (raise `--min-n`, or require the lift to repeat) — at N=4 with 100% divergence this is signal-or-noise and must not yet drive tuning.
- **Spirit note:** show, don't tell; the trigger is math, the enrichment is optional; tuning is still the LAST step. Live demo captured a real judge-vs-council-vs-human split + a RESPAWN ticket. See [`agora/README.md`](./agora/README.md) (re-entry loop), [`agora/watcher.py`](./agora/watcher.py), [`spikes/backyard/respawn.py`](./spikes/backyard/respawn.py).
